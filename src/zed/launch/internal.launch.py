# Copyright 2024 Stereolabs, 2025 Barrett Ray + the Sooner Rover Team
#
...
default_config_ffmpeg = os.path.join(
    get_package_share_directory("zed_wrapper"), "config", "ffmpeg.yaml"
)
...
def parse_array_param(param: str) -> list[str]:
    str = param.replace("[", "")
    str = str.replace("]", "")
    arr = str.split(",")
...
def launch_setup(
    ...
    return_array: list[LaunchDescriptionEntity] = []
    ...
    container_name_val: str = container_name.perform(context)
    namespace_val: str = namespace.perform(context)
    camera_name_val: str = camera_name.perform(context)
    camera_model_val: str = camera_model.perform(context)
    node_name_val: str = node_name.perform(context)
    enable_gnss_val: str = enable_gnss.perform(context)
    gnss_coords: list[str] = parse_array_param(
        gnss_antenna_offset.perform(context)
    )
    custom_baseline_val: str = custom_baseline.perform(context)
    ...
    if (
        camera_model_val == "zed"
        or camera_model_val == "zedm"
        or camera_model_val == "zed2"
        or camera_model_val == "zed2i"
        or camera_model_val == "zedx"
        or camera_model_val == "zedxm"
        or camera_model_val == "virtual"
    ):
    ...
    if ros_params_override_path_val != "":
        info = (
            "Using ROS parameters override file: "
    ...
    if container_name_val == "":
        ...
        container_exec: str = "component_container_isolated"
    ...
    # ZED Node parameters
    node_parameters: SomeParameters = [
        # YAML files
        ...
        #
        # add the launch file params directly
        {
    ...
    # we'll also add the `pointcloud_to_laserscan` component here
    pointcloud_to_laserscan = ComposableNode(
        ...
        parameters=[
            PathJoinSubstitution(
                [
                    get_package_share_directory("zed"),
                    "params",
        ...
        extra_arguments=[{"use_intra_process_comms": True}],
        remappings=[
            ("cloud_in", f"{camera_name_val}/depth/points"),
            ("scan", "/scan"),
        ],
    )
