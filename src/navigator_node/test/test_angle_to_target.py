...
def calc_angle(
    ...
    return calc_angle_to_target(
        target_coord, robot_coord_stamped, robot_heading
    )
...
def test_calc_target_north():
    ...
    robot_headings = [
        degree for degree in range(0, 361, 45)
    ...
    for robot_heading, expected_error in zip(robot_headings, expected_errors):
        assert (
            calc_angle(0.0, 1.0, robot_heading) == expected_error
        )  # far away target
        assert (
            calc_angle(0.0, 0.0001, robot_heading) == expected_error
        )  # close target
...
def test_calc_target_east():
    ...
    for robot_heading, expected_error in zip(robot_headings, expected_errors):
        assert (
            calc_angle(1.0, 0.0, robot_heading) == expected_error
        )  # far away target
        assert (
            calc_angle(0.0001, 0.0, robot_heading) == expected_error
        )  # close target
