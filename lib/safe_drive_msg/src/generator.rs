...
/// A trait that will be derived on some parent struct.
#[derive(Clone, Copy, Debug, PartialEq, PartialOrd, Ord, Eq)]
pub enum StructDerivedTrait {
    Clone,
    Debug,
}

impl StructDerivedTrait {
    /// Returns this trait's name.
    ///
    /// Used to create a type's derives.
    pub const fn trait_name(&self) -> &'static str {
        match self {
            StructDerivedTrait::Clone => "Clone",
            StructDerivedTrait::Debug => "Debug",
        }
    }
}

/// The default set of derived traits for a struct.
///
/// Currently, this set only includes `Debug`.
pub static DEFAULT_DERIVED_TRAITS_SET: LazyLock<BTreeSet<StructDerivedTrait>> =
    LazyLock::new(|| BTreeSet::from([StructDerivedTrait::Debug]));

/// The set of traits derived on the bindings for an action's result type.
///
/// Includes both `Clone` and `Debug`.
pub static ACTION_RESULT_DERIVED_TRAITS_SET: LazyLock<BTreeSet<StructDerivedTrait>> =
    LazyLock::new(|| BTreeSet::from([StructDerivedTrait::Clone, StructDerivedTrait::Debug]));
...
"#,
    ...
    fn generate_idl_msg(
        ...
        self.idl_struct(lines, struct_def, &DEFAULT_DERIVED_TRAITS_SET, lib);
    ...
    fn generate_idl_action(
        ...
        self.idl_struct(
            lines,
            struct_def,
            // note: actions are special in `safe_drive`. each `action_result`
            // MUST be `Clone` since:
            //
            // - `ActionMsg::ResultContent: TypeSupport + Clone`
            //
    ...
    fn idl_struct(
        ...
        lines: &mut VecDeque<String>,
        struct_def: &StructDef,
        derived_traits: &BTreeSet<StructDerivedTrait>,
        lib: &str,
    ) {
        lines.push_back("\n#[repr(C)]".to_string());
        lines.push_back({
            ...
            let max_idx: usize = derived_traits.len() - 1;
            for (idx, derived_trait) in derived_traits.iter().enumerate() {
                // map the trait into its name so we can derive it
                s.push_str(derived_trait.trait_name());
...
module example_msg {
    ...
    /// Action result messages must be `Clone` to work with `safe_drive`.
    ///
    /// So, let's check if the generator correctly handles that case!
    #[test]
    fn bind_action_result_correctly() {
        let mut gen: Generator<'static> = Generator::new(SafeDrive::Path("sample_path"));

        let trait_set_to_should_contain_clone: [(&LazyLock<BTreeSet<StructDerivedTrait>>, bool);
            2] = [
            (&DEFAULT_DERIVED_TRAITS_SET, false),
            (&ACTION_RESULT_DERIVED_TRAITS_SET, true),
        ];

        fn format_lines(vec: VecDeque<String>) -> String {
        ...
        for (trait_set, should_contain_clone) in trait_set_to_should_contain_clone {
            let mut lines: VecDeque<String> = VecDeque::new();

            gen.idl_struct(
                &mut lines,
                &StructDef {
                    id: format!("StructId_clone_{should_contain_clone}"),
                    members: Vec::new(),
                    inheritance: None,
                },
                trait_set,
            ...
            if should_contain_clone {
                assert_eq!(
                    r#"
#[repr(C)]
#[derive(Clone, Debug)]
pub struct StructId_clone_true {
}
"#,
                    format_lines(lines),
                );
...
#[repr(C)]
#[derive(Debug)]
pub struct StructId_clone_false {
}
"#,
                    format_lines(lines),
                );
